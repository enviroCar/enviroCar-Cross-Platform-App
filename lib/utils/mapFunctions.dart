...
LatLngBounds getLatLngBounds(LatLng startLocation, LatLng destinationLocation) {
  ...
  if (startLocation.latitude < destinationLocation.latitude &&
    ...
    latLngBounds = LatLngBounds(
        southwest:
            LatLng(startLocation.latitude, destinationLocation.longitude),
        northeast:
            LatLng(destinationLocation.latitude, startLocation.longitude));
...
Future<Set<Polyline>> setPolyLines(List<PointProperties> properties) async {
  ...
  if (len > 1) {
    for (final int i = 0; i < len - 1; len++) {
      ...
      final PolylineResult polylineResult = await polylinePoints
          .getRouteBetweenCoordinates(googleAPIKey, origin, destination,
