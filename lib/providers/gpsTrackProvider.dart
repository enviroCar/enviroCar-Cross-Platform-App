...
class GpsTrackProvider extends ChangeNotifier {
  ...
  Future setUpMap() async {
    ...
    NotificationService()
        .showNotifications('Track $trackId', 'GPS tracking has started!');
  ...
  Future setMapController(
      Future<GoogleMapController> googleMapController) async {
  ...
  void trackDuration([Timer timer]) {
    ...
    if (!isTrackingPaused) {
      ...
      duration = Duration(
          hours: hours.toInt(), minutes: minutes.toInt(), seconds: seconds);
      durationOfTrack =
  ...
  LocalTrackModel getLocalTrack({@required Car sensor}) {
    final LocalTrackModel trackModel = LocalTrackModel(
        ...
        stops: getNoOfStops,
        bluetoothDevice: BluetoothProvider().getConnectedDevice.name.toString(),
