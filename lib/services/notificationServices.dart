...
class NotificationService {
  ...
  Future initialiseNotificationPlugin() async {
    try {
      AwesomeNotifications().initialize(
          'resource://drawable/app_icon', [notificationChannel],
          debug: true);
    } catch (e) {
  ...
  Future showNotifications(String title, String body) async {
    ...
    const NotificationLifeCycle notificationLifeCycle =
    ...
    final NotificationActionButton notificationActionButton =
  ...
  void listenToNotification() {
    AwesomeNotifications().actionStream.listen((receivedNotification) {
      Navigator.pushReplacementNamed(
          navigatorKey.currentState.context, GpsTrackingScreen.routeName);
    });
  }
